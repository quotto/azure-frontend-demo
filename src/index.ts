...
import { FetchManager, RestApiResponse } from "./FetchManager";
...
export async function seeProfile(): Promise<void> {
    ...
    if (token && token.length > 0) {
        ...
        UIManager.updateUI(graphResponse as UserInfo | MailInfo, GRAPH_CONFIG.GRAPH_ME_ENDPT);
    }
}
...
export async function readMail(): Promise<void> {
    ...
    if (token && token.length > 0) {
        ...
        UIManager.updateUI(graphResponse as UserInfo | MailInfo, GRAPH_CONFIG.GRAPH_MAIL_ENDPT);
    }
}

export async function callGetUserProfileApi(): Promise<void> {
    const token = await authModule.getRestApiTokenPopup();
    if(token && token.length > 0) {
        const apiResponse = await networkModule.callEndpointWithToken("https://super-demo-func.azurewebsites.net/api/me", token);
        UIManager.updateUIRestApi(apiResponse as RestApiResponse, token, "delegate");
    }
}

export async function callGetAllTenantUsersApi(): Promise<void> {
    const token = await authModule.getRestApiTokenPopup();
    if(token && token.length > 0) {
        const apiResponse = await networkModule.callEndpointWithToken("https://super-demo-func.azurewebsites.net/api/users", token);
        UIManager.updateUIRestApi(apiResponse as RestApiResponse, token, "independent");
    }
}
export async function callGetUserApiWithoutToken(): Promise<void> {
    const apiResponse = await networkModule.callEndpointWithoutToken("https://super-demo-func.azurewebsites.net/api/users");
    UIManager.updateUIRestApi(apiResponse as RestApiResponse, "", "delegate");
}
/**
